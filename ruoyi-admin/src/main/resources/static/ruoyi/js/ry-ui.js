...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            // 当天数据统计
            statisticsGroupByProduct: function (title, width, height) {
                table.set();

                var _url = url = table.options.statisticsGroupByProduct;
                var _width = $.common.isEmpty(width) ? "800" : width;
                var _height = $.common.isEmpty(height) ? ($(window).height() - 50) : height;
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios|ipad)/i)) {
            ...
            updateBankCardMore: function (title, width, height) {
                table.set();

                var _url = url = table.options.updateBankCardMore;
                var _width = $.common.isEmpty(width) ? "800" : width;
                var _height = $.common.isEmpty(height) ? ($(window).height() - 50) : height;
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios|ipad)/i)) {
            ...
            exportSelect: function (title) {
                ...
                var url = table.options.exportUrlAll.replace("{ids}", rows.join());
                $.modal.loading("正在导出数据，请稍后...");
            ...
            approval: function (id) {
                ...
                if ($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.updateUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.approvalOpen("审核" + table.options.modalName, url);
                } else {
